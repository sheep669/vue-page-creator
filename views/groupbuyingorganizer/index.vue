...
export default {
    ...
    created() {
        this.getSubMenuData();
        //默认重定向到子路由的第一个path
        this.$router.push("/group_buying_organizer_manage");
    },
    methods: {
        getSubMenuData() {
            ...
            this.navData.forEach((v) => {
                //按需提取子路由
                if (v.name == "groupbuyingorganizer") {
            ...
            this.subMenuData.forEach((v2) => {
                v2.submenu.forEach((v3) => {
                    //按需添加子路由
                    this.$router.addRoute("groupbuyingorganizer", {
                        path: v3.path,
                        component: () =>
